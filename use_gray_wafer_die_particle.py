...
def _grid_profiles_color(image_bgr: np.ndarray,
    ...
    sat_roi = hsv[:, :, 1].astype(np.float32)
    valid_roi = (hsv[:, :, 2] >= val_min).astype(np.float32)
...
def detect_corner_grid(image_bgr: np.ndarray,
    ...
    gray_roi = cv2.cvtColor(image_bgr[y1:y2, x1:x2], cv2.COLOR_BGR2GRAY).astype(np.float32)
    blurred_low = cv2.GaussianBlur(gray_roi, (0, 0), sigmaX=3.0)
    sx_low = np.abs(cv2.Sobel(blurred_low, cv2.CV_32F, 1, 0, ksize=3))
    sy_low = np.abs(cv2.Sobel(blurred_low, cv2.CV_32F, 0, 1, ksize=3))
    pitch_x_rough = float(_autocorr_period(sx_low.mean(axis=0),
                                           min_lag=min_pitch, max_lag=max_pitch))
    pitch_y_rough = float(_autocorr_period(sy_low.mean(axis=1),
                                           min_lag=min_pitch, max_lag=max_pitch))
...
def detect_grid(image_bgr: np.ndarray,
    ...
    sx_low = np.abs(cv2.Sobel(blurred_low, cv2.CV_32F, 1, 0, ksize=3))
    sy_low = np.abs(cv2.Sobel(blurred_low, cv2.CV_32F, 0, 1, ksize=3))
    pitch_x_rough = _autocorr_period(sx_low.mean(axis=0),
                                     min_lag=min_pitch, max_lag=max_pitch)
    pitch_y_rough = _autocorr_period(sy_low.mean(axis=1),
                                     min_lag=min_pitch, max_lag=max_pitch)

    col_std, row_std = _grid_profiles_std(gray_roi)
...
def _wafer_silhouette(gray: np.ndarray, black_thr: int, open_ksize: int) -> np.ndarray:
    ...
    # Avoid allocating a full-size labels array on very large wafer images.
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return (mask > 0).astype(np.uint8)
    wafer_cnt = max(contours, key=cv2.contourArea)
    out = np.zeros(mask.shape, dtype=np.uint8)
    cv2.drawContours(out, [wafer_cnt], -1, 1, thickness=-1)
    return out
...
def clean_wafer(image_bgr: np.ndarray,
    ...
    keep = ((sil > 0) & disc).astype(np.uint8)
    keep *= 255
    out = image_bgr if in_place else image_bgr.copy()
    cv2.bitwise_and(out, out, dst=out, mask=keep)
    return out
...
def build_die_map(image: Union[str, Path, np.ndarray],
    ...
    if clean:
        img = clean_wafer(img, in_place=not isinstance(image, np.ndarray))
...
def render_overlay_portable(image_path: Union[str, Path], die_map: WaferDieMap,
                            overlay_path: Union[str, Path]) -> Path:
    """Draw the detected die rectangles and wafer circle on top of the input image."""
    ...
    ok = cv2.imwrite(str(overlay_path), img)
    if not ok:
...
def evaluate_bw_noisy_wafer(image_path: Union[str, Path],
    ...
    if overlay_path is None:
        overlay_path = image_path.with_name(image_path.stem + "_overlay.png")
    overlay_path = Path(overlay_path)

    die_map = build_die_map(
        str(image_path),
        grid_method=grid_method,
        angle_align_method=angle_align_method,
        clean=clean,
    )
...
def _portable_argparser():
    import argparse

    parser = argparse.ArgumentParser(
        description="Evaluate a noisy wafer image with the embedded Wafer Die Map V5 engine."
    )
    parser.add_argument("--image", required=True, help="Input wafer image path")
    parser.add_argument("--overlay", help="Output overlay image path")
    parser.add_argument("--grid-method", default="std", help="Grid method: corner | std | color | hybrid")
    parser.add_argument("--angle-align-method", default="die_render",
                        help="Angle align method: die_render | notch | vertical_line | none")
    parser.add_argument("--no-clean", action="store_true", help="Disable wafer cleanup before evaluation")
    return parser
...
def main() -> None:
    ...
    args = _portable_argparser().parse_args()
    result = evaluate_bw_noisy_wafer(
        args.image,
        args.overlay,
        grid_method=args.grid_method,
        angle_align_method=args.angle_align_method,
