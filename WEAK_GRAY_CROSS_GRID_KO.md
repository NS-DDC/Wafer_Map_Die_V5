...
4. 세로 ridge와 가로 ridge를 각각 검출한다. 물리적으로 1~2px인 street는 축소/보정 후 5px 정도 응답까지 허용한다.
5. 폭이 큰 세로 noise band는 세로 후보에서 제외한다. 넓은 noise의 양쪽 edge처럼 pitch 절반보다 가까운 두 ridge도 함께 제외한다. 남은 얇은 세로 ridge 중 wafer 중심에 가장 가까운 X와, 얇은 가로 ridge 중 wafer 중심 바로 위 Y를 조합해 corner `(x0,y0)`로 사용한다.
